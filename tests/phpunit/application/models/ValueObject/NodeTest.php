...
class Core_Model_ValueObject_NodeTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testAddAdditionalFieldShouldReturnFalseWithUnitializedNode()
    {
        $node = Core_Model_DiFactory::newNode();
        
        $this->assertFalse($node->addAdditionalField('key1', 'val1'));
    }
    ...
    {
        $node = $this->getMock('Core_Model_ValueObject_Node', array('save'), array('sample1'));
        $node->expects($this->once())
             ->method('save');
        
        $node->addAdditionalField('uno', 'one');
    }
